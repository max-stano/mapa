...
def nacitaj_mapu(cesta):
   ...
   #prehladavame maticu mapy (riadky, stlpce) a hladame umiestnenie postavičky a potvorky a v scene
    ...
    return sirka, vyska, mapa, pac_x, pac_y, duch_y, duch_x
#########################################################
#duchovia - aby sa zobrazilo viacero duchov
def dusi(platno, sirka, vyska, mapa, cell_size, duch_data):
    duchovia = []
    for y in range (0, vyska):
      for x in range (0, sirka):
            if mapa [y][x] == 'h':
                dus =  platno.create_image (x * cell_size + cell_size / 2, \
                                            y * cell_size + cell_size / 2, \
                                            image = duch_data)
                duchovia.append([x, y, dus])
               
dva = random.randrange(-80, 80)
tre = random.randrange(-80, 80)

def duch_pohyb(platno, duch_data, duchovia):
    for y in range (0, vyska):
        for x in range(0, sirka):
            platno.move(1, dva, tri)
            okno.update ()
            time.sleep(0.5)
            #neviem jak to rozpohybovať
    return duchovia

######################################################
def gulky(platno, sirka, vyska, mapa, cell_size, gulicka_data):
    gulicky = []
    for y in range (0, vyska):
      for x in range (0, sirka):
            if mapa [y][x] == 'g':
                gul = platno.create_image (x * cell_size + cell_size / 2, \
                                            y * cell_size + cell_size / 2, \
                                            image = gulicka_data)
                gulicky.append([x, y, gul])
...
#vsetky nazvy premennych sesdia s lokalnymi premennymi, ale nie su to totozne premenne. 
...
#duchovia = dusi(duch_data, mapa, sirka, vyska, platno, cell_size)
duchovia = dusi(platno, sirka, vyska, mapa, cell_size, duch_data)

gulicka_data = tkinter.PhotoImage (file= 'ball.gif')
gulicky = gulky(platno, sirka, vyska, mapa, cell_size, gulicka_data)
